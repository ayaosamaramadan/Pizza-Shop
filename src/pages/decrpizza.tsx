...
import { isDecModalOpen, remove, setSelectedPizza } from "@/store/pizzaSlice";
...
const Decrpizza = () => {
  ...
  const filteredItems = daupleitemCart?.filter((item) => item.id === selectedPizza?.id);

  if (isdecOpen && selectedPizza && filteredItems.length === 0) {
    closeModal();
  }
            ...
            <div className="fixed inset-0 flex items-center justify-center z-50 backdrop-blur-md ">
              <div className="bg-gradient-to-br from-white to-gray-100 p-8 rounded-xl shadow-2xl max-w-lg w-full max-h-[80vh] overflow-y-auto relative border border-gray-400 transform transition-transform duration-300 scale-105 ">
                ...
                {filteredItems.map((item, index) => (
                  <div
                    key={item.id}
                    className="mb-3 flex items-center space-x-6 relative bg-gray-50 p-5 rounded-lg shadow-md hover:shadow-xl transition-shadow duration-300 border border-gray-300 hover:bg-gray-100"
                  >
                    <Image
                      src={item.image}
                      alt={item.title}
